...
<div class="content-wrapper" style="background-color:#F5F5F5;">
  <div class="row mb-4 px-3">
    <div class="col-12 d-flex justify-content-between align-items-center">
      <h4 class="fw-bold" style="color:#3F51B5;">📱 Daftar Handphone</h4>
      <div>
        {{-- 🔹 Tombol Tambah --}}
        <a href="{{ route('handphone.create') }}" class="btn btn-primary rounded-3 shadow-sm" 
           style="background-color:#3F51B5; border:none;">
          <i class="mdi mdi-plus me-1"></i> Tambah Handphone
        </a>
        {{-- 🔹 Tombol Lihat Sampah (Trash) --}}
        <a href="{{ route('handphone.trash') }}" class="btn btn-outline-danger rounded-3 shadow-sm ms-2">
  ...
  <div class="card shadow-sm border-0 mx-3 rounded-4">
    <div class="card-header py-3 px-4" 
      ...
      <h5 class="mb-0 fw-bold">📋 Data Handphone</h5>
    </div>
    ...
    <div class="card-body bg-white p-4">
      ...
      {{-- 🔍 Form Pencarian --}}
      <form method="GET" action="{{ route('handphone.index') }}" class="mb-4">
        <div class="input-group" style="max-width: 350px;">
          <input type="text" name="search" class="form-control rounded-start-3"
                 placeholder="Cari brand / model / tahun..." value="{{ request('search') }}">
      ...
      <div class="table-responsive">
        <table class="table align-middle mb-0 custom-striped">
          ...
          <tbody>
            @forelse ($handphones as $index => $hp)
              <tr>
                <td>{{ $index + 1 }}</td>
                <td>
                  @if($hp->image)
                    <img src="{{ asset('storage/'.$hp->image) }}" 
                         class="rounded shadow-sm" width="55" height="55" style="object-fit:cover;">
                  @else
                    <img src="{{ asset('assets/img/default-phone.png') }}" 
                         class="rounded shadow-sm" width="55" height="55" style="object-fit:cover;">
                  @endif
                </td>
                <td>{{ $hp->brand }}</td>
                <td>{{ $hp->model }}</td>
                <td>{{ $hp->release_year }}</td>
                <td>
                  <span class="btn-status {{ $hp->is_active == 'active' ? 'active' : 'inactive' }}">
                    {{ ucfirst($hp->is_active) }}
                  </span>
                </td>
                <td class="text-center">
                  <a href="{{ route('handphone.show', $hp->id) }}" class="btn-action btn-detail me-1">Detail</a>
                  <a href="{{ route('handphone.edit', $hp->id) }}" class="btn-action btn-edit me-1">Edit</a>
                  <form action="{{ route('handphone.destroy', $hp->id) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
            ...
            @empty
              <tr>
                <td colspan="7" class="text-center text-muted py-4">Belum ada data handphone.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
