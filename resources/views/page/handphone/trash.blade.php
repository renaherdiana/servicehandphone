...
<div class="content-wrapper" style="background-color:#F5F5F5; min-height: 100vh;">
  <div class="container py-4">
    ...
    <div class="mb-4 d-flex justify-content-between align-items-center">
      <h4 class="fw-bold text-primary mb-0">🗑️ Trash Handphone</h4>
      <a href="{{ route('handphone.index') }}" class="btn btn-secondary rounded-3 shadow-sm">
    ...
    <div class="card shadow-sm border-0 rounded-4">
      <div class="card-header py-3 px-4"
        ...
        <h5 class="mb-0 fw-bold">📱 Data Handphone yang Dihapus</h5>
      </div>
          ...
          <table class="table align-middle mb-0 custom-striped">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  @if($hp->image)
                    <img src="{{ asset('storage/'.$hp->image) }}" 
                         class="rounded-circle shadow-sm" width="55" height="55" style="object-fit:cover;">
                  @else
                ...
                <td>{{ $hp->brand }}</td>
                <td>{{ $hp->model }}</td>
                <td>{{ \Carbon\Carbon::parse($hp->deleted_at)->translatedFormat('d F Y, H:i') }}</td>
                <td class="text-center">
                  {{-- 🔄 Tombol Restore --}}
                  <form action="{{ route('handphone.restore', $hp->id) }}" method="POST" class="d-inline">
                    @csrf
                    @method('PATCH')
                  ...
                  <form action="{{ route('handphone.forceDelete', $hp->id) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
