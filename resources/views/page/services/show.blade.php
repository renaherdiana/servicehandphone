        ...
        <div class="row mb-4">
          ...
          <div class="col-md-6 mb-3">
            ...
            <div class="border rounded-3 p-2 bg-light">
              {{ optional($service->customer)->name ?? '-' }}
            </div>
          </div>
          <div class="col-md-6 mb-3">
            <label class="fw-semibold text-primary">Handphone:</label>
            <div class="border rounded-3 p-2 bg-light">
              {{ optional($service->handphone)->brand ?? '-' }}
              {{ optional($service->handphone)->model ?? '' }}
            </div>
          </div>
          <div class="col-md-6 mb-3">
            <label class="fw-semibold text-primary">Teknisi:</label>
            <div class="border rounded-3 p-2 bg-light">
              {{ optional($service->technician)->name ?? '-' }}
            </div>
          </div>
          <div class="col-md-6 mb-3">
            ...
            <div class="border rounded-3 p-2 bg-light">
              @switch($service->status)
                ...
                @case('cancelled')
                  ...
                  @break
                @default
                  <span class="btn-status cancelled">Unknown</span>
              @endswitch
            </div>
          </div>
        </div>
